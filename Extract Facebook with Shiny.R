...
ui <- fluidPage(theme = shinytheme("slate"),
                ...
                fluidRow(tags$head(tags$style(type="text/css", "
                  ...
                  column(3, h4('1st STEP - Define the Page'),
                         ...
                         textInput('token','Insert your Facebook API token here', 'XXXX'),
                         textInput('page','Insert the name of the page you want to find', 'Nielsen Brasil'),
                  ...
                  column(3, h4('2nd STEP - Extract Posts'),
                         ...
                         numericInput('page_id', 'Selected Page ID',263920823618425),
                         numericInput('nposts', 'Number of posts to retrieve', 10),
                         dateInput('since', 'Retrieve posts since', value = '2016/02/24', min = '2016/02/24', format = 'yyyy/mm/dd'),
...
server<-function(input, output, session) {
  ...
  observeEvent(input$gopages, {
    values$df_pages <- searchPages(input$page, token = input$token, n = input$npages)
  ...
  observeEvent(input$retrieve, {
    values$df_data <- getPage(input$page_id, token = input$token, n=input$nposts, reactions = T, since = input$since)
